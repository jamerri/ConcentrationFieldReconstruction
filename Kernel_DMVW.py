...
'''1.File Reading'''
f = open("E:\\Desktop\\data\\5 percent random sampling data\\mixing ventilation\\DM_VW_5.txt", 'r', encoding='utf-8')
...
for i in range(0,len(measurement_data)):
...
temp_position_x = np.arange(0,6+cell_size_x,cell_size_x)
temp_position_y = np.arange(0,4.5+cell_size_y,cell_size_y)
...
for i in range(grid_num):
    ...
    for j in range(sample_num):
        ...
        rotation = np.array([(cos(wind_dir_rad), -sin(wind_dir_rad)),
                                  (sin(wind_dir_rad), cos(wind_dir_rad))])
        a = kernel_size + wind_scale * wind_speed[j]
        b = kernel_size / (1 + (wind_scale * wind_speed[j]) / kernel_size)
        ...
        rotation_distance = pow((rotation_sample_x - rotation_position_x), 2) / pow(radius_multiple * a, 2) + \
                            pow((rotation_sample_y - rotation_position_y), 2) / pow(radius_multiple * b, 2)
        if rotation_distance > 1:
            single_weight.append(0)
        else:
            covariance_matrix = np.array([(a * a, 0), (0, b * b)])
            rotation_covariance = np.dot(np.dot(rotation, covariance_matrix), np.transpose(rotation))
            sigma_x_sq = rotation_covariance[0][0]
            sigma_x = np.sqrt(sigma_x_sq)
            sigma_y_sq = rotation_covariance[1][1]
            sigma_y = np.sqrt(sigma_y_sq)
            sigma_xy = rotation_covariance[0][1]
            rho = sigma_xy / (sigma_x * sigma_y)
            coefficient_part = 1 / (2 * np.pi * sigma_x * sigma_y * np.sqrt(1 - rho ** 2))
            index_part = (((position_x[i] - sample_x[j]) ** 2 / sigma_x_sq) + ((position_y[i] - sample_y[j]) ** 2 / sigma_y_sq) -
                          ((2 * rho * (position_x[i] - sample_x[j]) * (position_y[i] - sample_y[j])) / (sigma_x * sigma_y)))
            temp_weight = coefficient_part * exp(-(0.5 / (1 - rho ** 2)) * index_part)
...
for i in range(len(weight)):
    ...
    if weight[i] + (1 - confidence[i]) * r0 == 0 or confidence[i] * weight_concentration[i] == 0:
        continue
    else:
...
file_path = desktop_path + '2D_KernelDMVW_' + time_stamp() + '.txt'
with open(file_path, 'w') as file:
    for i in range(len(position_x)):
        file.write(f"{position_x[i]}\t{position_y[i]}\t{mean_value[i]}\n")
print(f"mean_value saved: {file_path}")

normalized_file_path = desktop_path + '2D_KernelDMVW_normalize_' + time_stamp() + '.txt'
