...
'''1.File Reading'''
f = open("E:\\Desktop\\data\\10 percent random sampling data\\mixing ventilation\\DM_V_10.txt", 'r', encoding='utf-8')
...
temp_position_x = np.arange(0, 6.0 + cell_size_x, cell_size_x)
...
for i in range(grid_num):
    ...
    for j in range(sample_num):
        distance = pow((position_x[i] - sample_x[j]), 2) + pow((position_y[i] - sample_y[j]), 2)
...
'''6、Data saving'''
desktop_path = "E:\\Desktop\\Concentration field reconstruction results\\Kernel_DMV\\"
file_path = desktop_path + '2D_KernelDMV_' + time_stamp() + '.txt'
with open(file_path, 'w') as file:
    for i in range(len(position_x)):
        file.write(f"{position_x[i]}\t{position_y[i]}\t{mean_value[i]}\n")
print(f"mean_value saved: {file_path}")

normalized_file_path = desktop_path + '2D_KernelDMV_normalize_' + time_stamp() + '.txt'
